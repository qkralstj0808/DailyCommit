...
// 정렬 대상 범위 지정
Arrays.sort(arr1, 2, 5); // 인덱스 2부터 4까지만 정렬
System.out.println(Arrays.toString(arr1)); // 출력: [10,8,2,3,11,0]

// 2. 내림차순 {11, 10, 8, 3, 2, 0}
Arrays.sort(arr1, Collections.reverseOrder()); // int 배열 내림차순 정렬
Arrays.sort(stringArr, Collections.reverseOrder()); // String 배열 내림차순 정렬
...
public class StringExample {
    public static void main(String[] args) {
        ...
        // endsWith() 문자열이 특정 문자열로 끝나는지
        System.out.println(str.endsWith("rld")); // true

        // equals() 두 문자열이 동일한지 비교
        System.out.println(str.equals("Hello World")); // true

        // indexOf() 문자열 내에서 특정 문자열이 나타나는 첫 번째 위치를 반환, 없으면 -1을 반환
        ...
        // replace() 문자열 내의 특정 부분을 다른 문자열로 바꿈
        System.out.println(str.replace("World", "Java")); // Hello Java

        // toLowerCase() and toUpperCase() 문자열을 소문자나 대문자로 변환
        System.out.println(str.toLowerCase()); // hello world
        System.out.println(anotherStr.toLowerCase()); // hello
        System.out.println(str.toUpperCase()); // HELLO WORLD

        // trim() 문자열 앞뒤의 공백을 제거
        String spaceStr = "  Java  ";
        System.out.println(spaceStr.trim()); //Java

        // charAt() 문자열에서 특정 위치의 문자를 반환
        System.out.println(str.charAt(1)); // e

        // valueOf() 주어진 값을 문자열 형태로 변환
        int number = 30;
        System.out.println(String.valueOf(number) + " days"); // 30 days

        // contains() 문자열이 특정 문자열을 포함하고 있는지 확인
